...
func TestLauncherBuildsDeclaredPathSandbox(t *testing.T) {
	...
	for _, want := range []string{"--unshare-net", "--unshare-pid", "--unshare-ipc", "--unshare-uts", "--ro-bind", filepath.Join(workspace, "output"), "/workspace/output"} {
...
func TestNativeSandbox(t *testing.T) {
	...
	workspace := t.TempDir()
	p := capability.Profile{
		Filesystem: capability.Filesystem{Mode: capability.FilesystemWorkspaceWrite},
		Process:    capability.ProcessDeclaredCommand,
		Network:    capability.NetworkNone,
	}
	cmd, err := NewLauncher("").Command(p, workspace, "", []string{"/bin/sh", "-c", `printf allowed > /workspace/allowed; if printf blocked > /tmp/escape 2>/dev/null; then exit 1; fi; printf native-sandbox-ok`}, nil)
	if err != nil {
		t.Fatalf("Command() error = %v", err)
	}
	...
	content, err := os.ReadFile(filepath.Join(workspace, "allowed"))
	if err != nil {
